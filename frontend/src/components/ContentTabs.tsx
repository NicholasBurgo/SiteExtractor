    ...
    <div>
      ...
      {activeSubTab === 'media' && (
        <div className="space-y-6">
              ...
              {currentContent.media.images.map((image, index) => (
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-xs text-gray-500 truncate" title={image.url}>
                      {image.url}
                    </div>
                  </div>
                </div>
          ...
          {currentContent.media.gifs.length > 0 && (
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-4">GIFs</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {currentContent.media.gifs.map((gif, index) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-4">
                    ...
                    />
                    <div className="text-xs text-gray-500 truncate" title={gif.url}>
                      {gif.url}
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-4">
                {currentContent.media.videos.map((video, index) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="text-xs text-gray-500 truncate" title={video.url}>
                      {video.url}
                    </div>
                  </div>
                ))}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentContent.files.map((file, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-500 truncate max-w-xs" title={file.url}>
                        {file.url}
                      </td>
                    </tr>
                  ))}
      ...
      {activeSubTab === 'words' && (
        <div className="space-y-6">
          ...
          {/* Headings */}
          <div>
            <h3 className="text-lg font-medium text-gray-900 mb-4">Headings Structure</h3>
              ...
              {currentContent.words.headings.length === 0 ? (
                ...
                <div className="space-y-2">
                  {currentContent.words.headings.map((heading, index) => (
                    <div key={index} className={`text-sm ${heading.tag === 'h1' ? 'font-bold text-lg' : heading.tag === 'h2' ? 'font-semibold' : 'font-medium'}`}>
                      <span className="text-gray-500 text-xs">{heading.tag.toUpperCase()}:</span> {heading.text}
                    </div>
                  ))}
                </div>
              )}
          ...
          {/* Paragraphs */}
          <div>
            <h3 className="text-lg font-medium text-gray-900 mb-4">Paragraphs</h3>
          ...
          <div className="bg-blue-50 p-4 rounded">
            <div className="flex items-center justify-between">
              ...
              <span className="text-lg font-bold text-blue-900">{currentContent.words.wordCount}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {activeSubTab === 'links' && (
        <div className="space-y-6">
          ...
          {currentContent.links.broken.length > 0 && (
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-4">Broken Links</h3>
              <div className="bg-red-50 border border-red-200 rounded-md p-4">
                <div className="space-y-2">
                  {currentContent.links.broken.map((link, index) => (
                    <div key={index} className="text-sm">
                      <span className="text-red-600">{link.href}</span>
                      {link.status && <span className="text-red-500 ml-2">(Status: {link.status})</span>}
                    </div>
                  ))}
                </div>
              </div>
