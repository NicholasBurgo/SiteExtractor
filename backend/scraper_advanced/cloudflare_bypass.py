...
class CloudflareBypass:
    ...
    async def bypass_request(
        ...
        if self.cloudscraper_enabled:
            ...
            success, content, cookies = await self._try_cloudscraper(
    ...
    async def _try_cloudscraper(
        self, url: str, headers: Dict[str, str], proxy: str = None, timeout: int = 30
    ) -> Tuple[bool, Optional[str], Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.debug(
                    f"Cloudscraper failed with status {response.status_code} for {url}"
                )
                return False, None, None
    ...
    async def _try_flaresolverr(
        ...
        try:
            ...
            if proxy:
                request_data["proxy"] = {"url": proxy, "type": "http"}
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        logger.debug(
                            f"FlareSolverr request failed with status {response.status} for {url}"
                        )
                        return False, None, None
                    ...
                    if result.get("status") == "ok":
                        ...
                        for cookie in cookies:
                            if isinstance(cookie, dict):
                                cookie_dict[cookie.get("name", "")] = cookie.get(
                                    "value", ""
                                )
                        ...
                        logger.debug(
                            f"FlareSolverr challenge failed for {url}: {result.get('message', 'Unknown error')}"
                        )
                        return False, None, None
    ...
    async def get_cf_clearance_token(
