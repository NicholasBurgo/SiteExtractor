...
class FingerprintSpoofer:
    ...
    def generate_fingerprint(
        self, browser_profile: str = "chrome124"
    ) -> BrowserFingerprint:
        """Generate a complete browser fingerprint"""

        # Get base profile
        profile_config = self.config.get("browser_profiles", {}).get(
            browser_profile, {}
        )
        ...
        canvas_hash = (
            self._generate_canvas_hash()
            if self.config.get("canvas_noise", True)
            else ""
        )
        webgl_hash = (
            self._generate_webgl_hash() if self.config.get("webgl_noise", True) else ""
        )
    ...
    def _generate_canvas_hash(self) -> str:
        ...
        return hashlib.md5(json.dumps(noise_data, sort_keys=True).encode()).hexdigest()[
            :16
        ]
    ...
    def _generate_webgl_hash(self) -> str:
        ...
        return hashlib.md5(json.dumps(webgl_data, sort_keys=True).encode()).hexdigest()[
            :16
        ]
    ...
    def get_session_headers(
        self, fingerprint: BrowserFingerprint, referer: Optional[str] = None
    ) -> Dict[str, str]:
