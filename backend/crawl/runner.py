...
class RunManager:
    ...
    async def start(self, req) -> str:
        ...
        bot_enabled = (
            bool(req.botAvoidanceEnabled)
            if hasattr(req, "botAvoidanceEnabled")
            else False
        )
        store = RunStore(run_id, meta_overrides={"botAvoidanceEnabled": bot_enabled})
        confirmation_store = ConfirmationStore(run_id)
        frontier = Frontier(
            req.url, max_pages=req.maxPages or settings.MAX_PAGES_DEFAULT
        )
        ...
        self._runs[run_id] = RunState(
            ...
            confirmation_store,
            time.time(),
            req.maxPages or settings.MAX_PAGES_DEFAULT,
            bot_strategy,
        )
            ...
            async with sem:
                ...
                if ct.startswith("text/html"):
                    ...
                    if url == state.frontier.start_url:
                        try:
                            state.confirmation_store.extract_site_data(
                                resp.content.decode("utf-8", errors="ignore"), url
                            )
                        ...
                        state.confirmation_store.add_page_to_index(
                            {
                                ...
                                "contentLengthBytes": summary.get(
