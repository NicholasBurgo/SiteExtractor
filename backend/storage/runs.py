...
class RunStore:
    ...
    def __init__(
        self, run_id: str, data_dir: str = "runs", meta_overrides: dict | None = None
    ):
    ...
    def log_error(self, url: str, error_type: str):
        ...
        try:
            ...
            meta["errors"].append(
                {"url": url, "error_type": error_type, "timestamp": time.time()}
            )
...
def compute_performance_summary(pages: List[PageResult]) -> Dict[str, Any]:
    ...
    successful_pages = [
        page for page in pages if page.status == 200 and page.load_time_ms is not None
    ]
    ...
    total_load = sum(
        page.load_time_ms for page in successful_pages if page.load_time_ms is not None
    )
    ...
    return {
        ...
        "fastest": {"url": fastest_page.url, "load_ms": fastest_page.load_time_ms},
        "slowest": {"url": slowest_page.url, "load_ms": slowest_page.load_time_ms},
    }
