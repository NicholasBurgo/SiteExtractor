...
async def get_extraction_status(run_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error checking extraction status: {str(e)}"
        )
...
async def get_prime_data(run_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting prime data: {str(e)}"
        )


@router.get("/{run_id}/content")
async def get_page_content(
    run_id: str, page_path: str = Query(..., description="Page path to get content for")
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting page content: {str(e)}"
        )
...
async def update_navigation(run_id: str, nav: List[Dict[str, Any]]):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error updating navigation: {str(e)}"
        )
...
async def update_page_content(
    run_id: str,
    page_path: str = Query(..., description="Page path to update"),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error updating page content: {str(e)}"
        )
