...
class BusinessAggregator:
    ...
    def _create_empty_draft(self) -> DraftModel:
        ...
        return DraftModel(
            ...
            sitemap={"primary": [], "secondary": [], "footer": []},
        )
    ...
    def _extract_business_profile(self) -> BusinessProfile:
        ...
        # Set sources
        profile.sources = [
            page.summary.pageId for page in self.pages[:3]
        ]  # First 3 pages
    ...
    def _extract_locations(self) -> List[Location]:
        ...
        for loc in locations:
            ...
            if (
                key not in unique_locations
                or loc.confidence > unique_locations[key].confidence
            ):
    ...
    def _is_menu_page(self, page: PageDetail) -> bool:
        ...
        return any(
            keyword in url_lower or keyword in title_lower for keyword in menu_keywords
        )
    ...
    def _is_team_page(self, page: PageDetail) -> bool:
        ...
        return any(
            keyword in url_lower or keyword in title_lower for keyword in team_keywords
        )
    ...
    def _extract_items_from_page(
        self, page: PageDetail, item_type: str
    ) -> List[ItemBase]:
        ...
        for i, heading in enumerate(page.headings):
            if isinstance(heading, str):
                ...
                item = ItemBase(
                    id=hashlib.md5(
                        f"{page.summary.pageId}_{title}".encode()
                    ).hexdigest()[:8],
                    title=title,
    ...
    def _extract_description_after_heading(
        self, page: PageDetail, heading_index: int
    ) -> Optional[str]:
    ...
    def _calculate_item_confidence(
        self, page: PageDetail, title: str, item_type: str
    ) -> float:
    ...
    def _deduplicate_items(
        self, items: List[ItemBase], item_type: str
    ) -> List[ItemBase]:
    ...
    def _extract_location_from_page(self, page: PageDetail) -> Optional[Location]:
        ...
        if addresses:
            return Location(
                id=hashlib.md5(f"{page.summary.pageId}_location".encode()).hexdigest()[
                    :8
                ],
